...
2. `https://unusualwhales.com/stock/SPX/greek-exposure?tab=Gamma`
   ...
   - `usable_for_dom_reader`: `true`
   - `confidence`: `0.9`
   - `reason`: Gamma 子标签更聚焦 GEX，可优先验证 gamma 相关 DOM 的稳定读取能力。
...
**next step:** 先用登录后的本地浏览器确认主页面与 Gamma 子页有无共用同一套 DOM 结构。
...
**next step:** 先在登录浏览器里确认 `flow/overview` 能稳定定位 SPX/SPY/QQQ 的实时流区域；docs 只作为字段释义。
...
1. `https://unusualwhales.com/stock/SPX/nope`
   ...
   - `usable_for_dom_reader`: `unknown`
   - `confidence`: `0.66`
   - `reason`: SPX 专用 NOPE 页面已存在，但核心字段可见性仍需登录后确认。
...
**next step:** 人工登录确认 `SPX/SPY NOPE` 页面里有无 `call delta / put delta / stock volume / divergence or overheat` 再决定能不能写 Reader。
...
**next step:** 先在登录页面确认 `term structure` 与 `IV/realized volatility` 位于同一页面，然后决定直接接 SPX 单页还是继续人工确认。
