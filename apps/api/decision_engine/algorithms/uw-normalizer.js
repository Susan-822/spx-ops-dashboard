...
function normalizeVolatility(raw = {}, provider = {}) {
  ...
  const frontIv = firstNumberFromRows({ data: termRows.slice(0, 1) }, ['iv', 'implied_volatility']);
  const backIv = firstNumberFromRows({ data: termRows.slice(-1) }, ['iv', 'implied_volatility']);
  const missingFields = [
...
function normalizeDealer(raw = {}) {
  ...
  return {
    greek_exposure_has_data: Object.keys(greekRow).length > 0,
...
export function buildUwNormalized({ raw = {}, provider = {} } = {}) {
  return {
    dealer: normalizeDealer(raw),
    flow: normalizeFlow(raw),
    volatility: normalizeVolatility(raw, provider),
    darkpool: normalizeDarkpool(raw),
    sentiment: normalizeSentiment(raw),
    data_health: {
