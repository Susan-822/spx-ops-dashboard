...
const REGIME_TEMPLATES = {
  ...
  [REGIME.BULL_TRAP]: {
    ...
    action: `绝对禁止追高！准备在 ${ctx.callWall ?? '--'} 附近寻找受阻做空机会，或等待回踩 ${ctx.localPutWall ?? '--'}。`,
    hard_stop_direction: 'BEAR',
    allow_trade: false,
    force_wait: true,
  },
  [REGIME.GRINDER]: {
    ...
    action: `主力在耗时间杀期权权利金（Theta）。管住手，放弃突破幻想，只做 ${ctx.localPutWall ?? '--'}–${ctx.localCallWall ?? '--'} 区间的高抛低吸，或直接空仓看戏。`,
    hard_stop_direction: 'NEUTRAL',
    allow_trade: false,
    force_wait: false,
  },
  [REGIME.INSTITUTIONAL_BUY]: {
    ...
    action: `多头动能确认！回踩 ${ctx.localPutWall ?? '--'} 企稳即是买点，目标看至 ${ctx.localCallWall ?? '--'}。`,
